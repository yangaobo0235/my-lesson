...
// Agent calls may include model and tool execution, so they need a separate budget.
export const AI_AXIOS = axios.create({
    baseURL: GATEWAY_HOST,
    timeout: 60000
});
...
AI_AXIOS.interceptors.request.use(
    attachToken,
    err => Promise.reject(err)
);
...
AI_AXIOS.interceptors.response.use(
    req => {
        return req;
    },
    err => Promise.reject(err)
